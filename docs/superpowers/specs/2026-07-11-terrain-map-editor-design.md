...
Selection follows the specified nonstandard contract: with an existing selection, a normal click on another unselected surface only clears; it does not select the new surface in that click. Ctrl toggles membership and survives key release. Brush records a fixed add/subtract mode from its first surface and visits each surface once. Touch exposes explicit add/subtract modes. Empty-space Select drag creates a viewBox marquee; polygon/segment intersection uses transformed surface boundaries rather than screen boxes.
